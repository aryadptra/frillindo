                    ...
                    <div class="flex overflow-x-scroll hide-scroll-bar dragscroll">
                        ...
                        <div class="flex mt-2 flex-nowrap">
                            @forelse ($thumbnail as $item)
                                <img :class="{ 'border-4 border-serv-button': active === {{ $item->id }} }" @click="changeThumbnail('{{ url(Storage::url($item->thumbnail)) }}', {{ $item->id }})" src="{{ Storage::url($item->thumbnail) }}" alt="thumbnail service" class="inline-block mr-2 rounded-lg cursor-pointer h-20 w-36 object-cover">
                            @empty
                                {{-- empty --}}
                            @endforelse
                        </div>
